          ...
          if (use_implicit_solver(j)) then
            ...
            if (proc%cart_dims(1) == 1) then
              ...
              if (proc%cart_coords(1) == 0) then 
                ...
                rhs_all(1:proc_length) = rhs

                do i = 2 , proc%cart_dims(1) 
                  !write(*,*) 'root' , proc%id , '<----' , proc%id + proc%cart_dims(2) * (i-1) , (i - 1) * proc_length + 1 ,  i  * proc_length
                  call MPI_RECV(rhs_all((i - 1) * proc_length + 1 : i  * proc_length ) , proc_length , MPI_DOUBLE , proc%id + proc%cart_dims(2) * (i-1)  , 100 , proc%comm , status,ierr)
                end do
                call zonal_solver(j,k)%solve(rhs_all, u_all)
                do i = 2 , proc%cart_dims(1) 
                  call MPI_SEND(u_all((i - 1) * proc_length + 1 : i  * proc_length) , proc_length , MPI_DOUBLE , proc%id + proc%cart_dims(2) * (i-1) , 100 , proc%comm , status,ierr)
                end do
                state%u(mesh%half_lon_ibeg:mesh%half_lon_iend,j,k) = u_all(1:proc_length)
              else
                !write(*,*) 'other' , proc%id , '---->' , proc%id - proc%cart_dims(2) * proc%cart_coords(1) , mesh%half_lon_ibeg   , mesh%half_lon_iend 
                call MPI_SEND(rhs(block%mesh%half_lon_ibeg:block%mesh%half_lon_iend) , size(rhs) , MPI_DOUBLE , proc%id - proc%cart_dims(2) * proc%cart_coords(1) , 100 , proc%comm , ierr)
                call MPI_RECV(state%u(mesh%half_lon_ibeg:mesh%half_lon_iend,j,k) , proc_length , MPI_DOUBLE , proc%id - proc%cart_dims(2) * proc%cart_coords(1) , 100 , proc%comm , status,ierr)
              end if
            end if 
          else
            do i = mesh%half_lon_ibeg, mesh%half_lon_iend

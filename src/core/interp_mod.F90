...
contains
  ...
  subroutine interp_cell_to_lon_edge(mesh, x, x_lon, reversed_area, u)
    ...
    real(r8), intent(inout) :: x_lon(member_num ,                       &
                                     mesh%half_lon_lb:mesh%half_lon_ub, &
                                     mesh%full_lat_lb:mesh%full_lat_ub, &
                                     mesh%full_lev_lb:mesh%full_lev_ub)
  ...
  subroutine interp_cell_to_vtx(mesh, x, x_vtx)
    ...
    integer i, j, k
    real(r8) pole(member_num ,mesh%num_full_lev)
    !real(r8) order_reduce(member_num , mesh%full_lon_ibeg, mesh%full_lon_iend)
      ...
      do j = mesh%full_lat_ibeg, mesh%full_lat_iend
        do i = mesh%full_lon_ibeg, mesh%full_lon_iend
          do k = mesh%full_lev_iend, mesh%full_lev_ibeg + 1, -1
            if (p(im,i,j,k) >= po .and. p(im,i,j,k-1) <= po) then
              if (logp_) then
  ...
  subroutine interp_lon_edge_to_isobaric_level(mesh, p, x, po, y, logp)
    ...
    do im = 1 , member_num
      do j = mesh%full_lat_ibeg, mesh%full_lat_iend
        do i = mesh%half_lon_ibeg, mesh%half_lon_iend
          do k = mesh%full_lev_iend, mesh%full_lev_ibeg + 1, -1
            p1_lon = 0.5_r8 * (p(im,i,j,k-1) + p(im,i+1,j,k-1))
            p2_lon = 0.5_r8 * (p(im,i,j,k  ) + p(im,i+1,j,k  ))
...
#else
            p1_lat = 0.5_r8 * (p(im,i,j,k-1) + p(im,i,j+1,k-1))
            p2_lat = 0.5_r8 * (p(im,i,j,k  ) + p(im,i,j+1,k  ))
#endif

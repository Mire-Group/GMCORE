...
contains
  ...
  subroutine operators_prepare_2(block, state, dt, pass)
    ...
    if (pass == vor_damp_pass) then
      ...
      call calc_m_lon_m_lat         (block, state) !need output fillhalo
    ...
    if (baroclinic) then
      ...
      do k = mesh%full_lev_ibeg, mesh%full_lev_iend
        do j = mesh%full_lat_ibeg, mesh%full_lat_iend
          do i = mesh%full_lon_ibeg, mesh%full_lon_iend
            do im = 1 , member_num
              state%t(im,i,j,k) = temperature(state%pt(im,i,j,k), state%ph(im,i,j,k))
            end do
          end do
        end do
      end do
      ...
      do k = mesh%half_lev_ibeg + 1, mesh%half_lev_iend - 1
        do j = mesh%full_lat_ibeg, mesh%full_lat_iend
          do i = mesh%full_lon_ibeg, mesh%full_lon_iend
            ...
            do im = 1 , member_num
              state%wedphdlev_lev(im,i,j,k) = - vert_coord_calc_dphdt_lev(k, tend%dphs(im,i,j)) - mf(im)
            end do
          end do
        end do
      end do
...
#else
      call fill_halo_member(block, state%wedphdlev_lev, full_lon=.true., full_lat=.true., full_lev=.false., west_halo=.false., south_halo=.false.)
#endif
  ...
  call Get_Start_Time(tran_time_start)
    ...
    if (div_damp_order == 4) then
      call fill_halo_member(block, state%div, full_lon=.true., full_lat=.true., full_lev=.true.)
    else
#ifdef V_POLE
      !call fill_halo(block, state%div, full_lon=.true., full_lat=.true., full_lev=.true., west_halo=.false., north_halo=.false.)
#else
      call fill_halo_member(block, state%div, full_lon=.true., full_lat=.true., full_lev=.true., west_halo=.false., south_halo=.false.)
...
#else
      call fill_halo_member(block, state%div2, full_lon=.true., full_lat=.true., full_lev=.true., west_halo=.false., south_halo=.false.)
  ...
  call Get_Start_Time(tran_time_start)
      ...
      call fill_halo_member(block, state%gz_lev, full_lon=.true., full_lat=.true., full_lev=.false.)
  ...
  call Get_Start_Time(tran_time_start)
    ...
    call fill_halo_member(block, state%m, full_lon=.true., full_lat=.true., full_lev=.true.)
  ...
  subroutine calc_m_lon_m_lat(block, state)
    ...
    call fill_halo_member(block, state%m_lon, full_lon=.false., full_lat=.true., full_lev=.true.)
    call fill_halo_member(block, state%m_lat, full_lon=.true., full_lat=.false., full_lev=.true.)
  ...
  call Get_Start_Time(tran_time_start)
      ...
      call fill_halo_member(block, state%pt_lon, full_lon=.false., full_lat=.true., full_lev=.true.)
...
#else
      call fill_halo_member(block, state%pt_lat, full_lon=.true., full_lat=.false., full_lev=.true., north_halo=.false.)
  ...
  call Get_Start_Time(tran_time_start)
    ...
    call fill_halo_member(block, state%mf_lon_n, full_lon=.false., full_lat=.true., full_lev=.true.)
  ...
  call Get_Start_Time(tran_time_start)
    ...
    call fill_halo_member(block, state%mf_lat_n, full_lon=.true., full_lat=.false., full_lev=.true.)
    ...
    do k = mesh%full_lev_ibeg, mesh%full_lev_iend
      do j = mesh%half_lat_ibeg, mesh%half_lat_iend
        if (block%reduced_mesh(j)%reduce_factor > 1) then
          ...
          call overlay_inner_halo_member(block, tend%qhu(:,:,j,k), west_halo=.true.)
        else
          do i = mesh%full_lon_ibeg, mesh%full_lon_iend
            if (coriolis_scheme == 1) then
              tend%qhu(:,i,j,k) = (                                                             &
        ...
        if (block%reduced_mesh(j+1)%reduce_factor > 1) then
          call zero_halo_member(block, tend%qhu(:,:,j,k), east_halo=.true.)
          do move = 1, block%reduced_mesh(j+1)%reduce_factor
            do i = block%reduced_mesh(j+1)%full_lon_ibeg, block%reduced_mesh(j+1)%full_lon_iend
              block%reduced_tend(j+1)%qhu(:,i,k) = (                     &
    ...
    do k = mesh%full_lev_ibeg, mesh%full_lev_iend
      do j = mesh%full_lat_ibeg_no_pole, mesh%full_lat_iend_no_pole
        if (block%reduced_mesh(j)%reduce_factor > 1) then
          ...
          call overlay_inner_halo_member(block, tend%qhv(:,:,j,k), west_halo=.true.)
        else
          do i = mesh%half_lon_ibeg, mesh%half_lon_iend
            if (coriolis_scheme == 1) then
              tend%qhv(:,i,j,k) = (                                                           &
    ...
    do k = mesh%full_lev_ibeg, mesh%full_lev_iend
      do j = mesh%full_lat_ibeg_no_pole, mesh%full_lat_iend_no_pole
        if (do_reduce_ke .and. block%reduced_mesh(j)%reduce_factor > 1) then
          ...
          call overlay_inner_halo_member(block, tend%dkedlon(:,:,j,k), west_halo=.true.)
        else
          do i = mesh%half_lon_ibeg, mesh%half_lon_iend
    ...
    do k = mesh%full_lev_ibeg, mesh%full_lev_iend
      do j = mesh%full_lat_ibeg_no_pole, mesh%full_lat_iend_no_pole
        if (block%reduced_mesh(j)%reduce_factor > 1) then
          ...
          call overlay_inner_halo_member(block, tend%dmfdlon(:,:,j,k), west_halo=.true.)
        else
          do i = mesh%full_lon_ibeg, mesh%full_lon_iend
            tend%dmfdlon(:,i,j,k) = (                           &
      ...
      do k = mesh%full_lev_ibeg, mesh%full_lev_iend
        do j = mesh%full_lat_ibeg_no_pole, mesh%full_lat_iend_no_pole
          if (block%reduced_mesh(j)%reduce_factor > 1) then
            ...
            call overlay_inner_halo_member(block, tend%dptfdlon(:,:,j,k), west_halo=.true.)
          else
            do i = mesh%full_lon_ibeg, mesh%full_lon_iend
              tend%dptfdlon(:,i,j,k) = (                            &

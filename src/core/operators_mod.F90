...
contains
  ...
  subroutine operators_prepare_2(block, state, dt, pass)
    ...
    if (pass == vor_damp_pass) then
      ...
      if (baroclinic) then  ! 斜压
        call diag_ph                  (block, state)  ! 从phs计算半层的ph_lev的整层的ph
        call interp_pt                (block, state)  ! 将位温插值到lon_edge，lat_edge和lev_edge上
        call diag_t                   (block, state)  ! 从pt算绝对的温度t
      end if
      call diag_m                     (block, state)  ! 从半层的lev_ph计算整层的layer_ph
      if (pass /= no_wind_pass) then  ! 需要计算风场有关的倾向
      ...
      if (hydrostatic) then ! 静力的gz直接可以诊断出来
        call diag_gz_lev              (block, state)
      end if
      call pgf_prepare                (block, state)  ! 准备水平气压梯度力？
    ...
    do k = mesh%half_lev_ibeg, mesh%half_lev_iend
      do j = mesh%full_lat_ibeg, mesh%full_lat_iend
        do i = mesh%full_lon_ibeg, mesh%full_lon_iend
          state%ph_lev(i,j,k) = vert_coord_calc_ph_lev(k, state%phs(i,j)) ! 半层的ph
        end do
      end do
    end do
  ...
  subroutine diag_m(block, state) ! m: δΠ, i.e., layer_ph
    ...
    if (baroclinic) then
      do k = mesh%full_lev_ibeg, mesh%full_lev_iend
        do j = mesh%full_lat_ibeg, mesh%full_lat_iend
          do i = mesh%full_lon_ibeg, mesh%full_lon_iend
            state%m(i,j,k) = state%ph_lev(i,j,k+1) - state%ph_lev(i,j,k)  ! layer_ph
            #ifndef NDEBUG
  ...
  subroutine interp_pt(block, state)  ! 将cell的pt插值到lon_edge，lat_edge和lev_edge上
  ...
  subroutine interp_m_vtx(block, state) ! 将m从网格中心（整层）插值到整层的网格顶点

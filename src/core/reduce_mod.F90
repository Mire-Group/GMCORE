...
contains
    ...
    do iblk = 1, size(blocks)
      ...
      do j = blocks(iblk)%mesh%full_lat_ibeg - 1, blocks(iblk)%mesh%full_lat_iend + 1
        if (blocks(iblk)%reduced_mesh(j)%reduce_factor > 0) then
          ...
          call allocate_reduced_static(blocks(iblk)%reduced_mesh(j), blocks(iblk)%reduced_static(j),member_num)
          ...
          call allocate_reduced_state(blocks(iblk)%reduced_mesh(j), blocks(iblk)%reduced_state(j),member_num)
          call allocate_reduced_tend(blocks(iblk)%reduced_mesh(j), blocks(iblk)%reduced_tend(j),member_num)
        end if
      end do
    end do
  ...
  subroutine allocate_reduced_state(reduced_mesh, reduced_state , member_num)
...
#define full_lev_full_lon_dims(x, buf_bound) reduced_state%x(member_num, reduced_mesh%full_lev_lb:reduced_mesh%full_lev_ub,reduced_mesh%full_lon_lb:reduced_mesh%full_lon_ub,buf_bound,reduced_mesh%reduce_factor)
#define full_lev_half_lon_dims(x, buf_bound) reduced_state%x(member_num, reduced_mesh%full_lev_lb:reduced_mesh%full_lev_ub,reduced_mesh%half_lon_lb:reduced_mesh%half_lon_ub,buf_bound,reduced_mesh%reduce_factor)
#define half_lev_full_lon_dims(x, buf_bound) reduced_state%x(member_num, reduced_mesh%half_lev_lb:reduced_mesh%half_lev_ub,reduced_mesh%full_lon_lb:reduced_mesh%full_lon_ub,buf_bound,reduced_mesh%reduce_factor)
...
#define reduce_args(x, sub) lbound(reduced_state%x, 4), ubound(reduced_state%x, 4), j, block, raw_mesh, raw_state, reduced_mesh, reduced_static, reduced_state, sub, dt
  ...
  subroutine reduce_gzs(j, buf_j, move, block, raw_mesh, raw_static, reduced_mesh, reduced_static)
    ...
    if (raw_mesh%is_inside_with_halo_full_lat(j+buf_j)) then
      ...
      do i = reduced_mesh%full_lon_ibeg, reduced_mesh%full_lon_iend
        do im = 1 , member_num
          reduced_static%gzs(im,i,buf_j,move) = sum(raw_static%gzs(im,raw_i:raw_i+reduced_mesh%reduce_factor-1,j+buf_j) * reduced_mesh%weights)
        end do
        raw_i = raw_i + reduced_mesh%reduce_factor
      end do
      call fill_zonal_halo_member(block, reduced_mesh%halo_width, reduced_static%gzs(:,:,buf_j,move))
    end if
  ...
  subroutine reduce_u(j, buf_j, move, block, raw_mesh, raw_state, reduced_mesh, reduced_static, reduced_state, dt)
    ...
    call fill_zonal_halo_member(block, reduced_mesh%halo_width, reduced_state%u(:,:,:,buf_j,move))
  ...
  subroutine reduce_v(j, buf_j, move, block, raw_mesh, raw_state, reduced_mesh, reduced_static, reduced_state, dt)
    ...
    call fill_zonal_halo_member(block, reduced_mesh%halo_width, reduced_state%v(:,:,:,buf_j,move))
  ...
  subroutine reduce_gz(j, buf_j, move, block, raw_mesh, raw_state, reduced_mesh, reduced_static, reduced_state, dt)
    ...
    if (raw_mesh%is_inside_with_halo_full_lat(j+buf_j)) then
      ...
      do i = reduced_mesh%full_lon_ibeg, reduced_mesh%full_lon_iend
        do k = reduced_mesh%full_lev_ibeg, reduced_mesh%full_lev_iend
          do im = 1 , member_num
            reduced_state%gz(im,k,i,buf_j,move) = sum(raw_state%gz(im,raw_i:raw_i+reduced_mesh%reduce_factor-1,j+buf_j,k) * reduced_mesh%weights)
          end do
        end do
        raw_i = raw_i + reduced_mesh%reduce_factor
      end do
      call fill_zonal_halo_member(block, reduced_mesh%halo_width, reduced_state%gz(:,:,:,buf_j,move), west_halo=.false.)
    end if
    ...
    if (raw_mesh%is_inside_with_halo_full_lat(j+buf_j)) then
      ...
      do i = reduced_mesh%full_lon_ibeg, reduced_mesh%full_lon_iend
        do k = reduced_mesh%half_lev_ibeg, reduced_mesh%half_lev_iend
          do im = 1 , member_num
            reduced_state%gz_lev(im,k,i,buf_j,move) = sum(raw_state%gz_lev(im,raw_i:raw_i+reduced_mesh%reduce_factor-1,j+buf_j,k) * reduced_mesh%weights)
          end do
        end do
        raw_i = raw_i + reduced_mesh%reduce_factor
      end do
      call fill_zonal_halo_member(block, reduced_mesh%halo_width, reduced_state%gz_lev(:,:,:,buf_j,move), west_halo=.false.)
    end if
  ...
  subroutine reduce_m(j, buf_j, move, block, raw_mesh, raw_state, reduced_mesh, reduced_static, reduced_state, dt)
    ...
    if (raw_mesh%is_inside_with_halo_full_lat(j+buf_j)) then
      ...
      do i = reduced_mesh%full_lon_ibeg, reduced_mesh%full_lon_iend
        do k = reduced_mesh%full_lev_ibeg, reduced_mesh%full_lev_iend
          do im = 1 , member_num
            reduced_state%m(im,k,i,buf_j,move) = sum(raw_state%m(im,raw_i:raw_i+reduced_mesh%reduce_factor-1,j+buf_j,k) * reduced_mesh%weights)
          end do
        end do
        raw_i = raw_i + reduced_mesh%reduce_factor
      end do
      call fill_zonal_halo_member(block, reduced_mesh%halo_width, reduced_state%m(:,:,:,buf_j,move))
    end if
  ...
  subroutine reduce_pv(j, buf_j, move, block, raw_mesh, raw_state, reduced_mesh, reduced_static, reduced_state, dt)
    ...
    if (reduce_pv_directly) then
      ...
      if (.not. raw_mesh%is_outside_pole_half_lat(j+buf_j)) then
        ...
        do i = reduced_mesh%half_lon_ibeg, reduced_mesh%half_lon_iend
          do k = reduced_mesh%full_lev_ibeg, reduced_mesh%full_lev_iend
            do im = 1 , member_num
              reduced_state%pv(im,k,i,buf_j,move) = sum(raw_state%pv(im,raw_i:raw_i+reduced_mesh%reduce_factor-1,j+buf_j,k) * reduced_mesh%weights)
            end do  
          end do
          raw_i = raw_i + reduced_mesh%reduce_factor
        end do
    ...
    call fill_zonal_halo_member(block, reduced_mesh%halo_width, reduced_state%pv(:,:,:,buf_j,move))
  ...
  subroutine reduce_m_lon(j, buf_j, move, block, raw_mesh, raw_state, reduced_mesh, reduced_static, reduced_state, dt)
    ...
    call fill_zonal_halo_member(block, reduced_mesh%halo_width, reduced_state%m_lon(:,:,:,buf_j,move), east_halo=.false.)
  ...
  subroutine reduce_m_lat(j, buf_j, move, block, raw_mesh, raw_state, reduced_mesh, reduced_static, reduced_state, dt)
    ...
    call fill_zonal_halo_member(block, reduced_mesh%halo_width, reduced_state%m_lat(:,:,:,buf_j,move), west_halo=.false.)
  ...
  subroutine reduce_mf_lon_n(j, buf_j, move, block, raw_mesh, raw_state, reduced_mesh, reduced_static, reduced_state, dt)
    ...
    if (raw_mesh%is_inside_with_halo_full_lat(j+buf_j)) then
      ...
      do i = reduced_mesh%half_lon_ibeg, reduced_mesh%half_lon_iend
        do k = reduced_mesh%full_lev_ibeg, reduced_mesh%full_lev_iend
          do im = 1 , member_num
            reduced_state%mf_lon_n(im,k,i,buf_j,move) = sum(raw_state%mf_lon_n(im,raw_i:raw_i+reduced_mesh%reduce_factor-1,j+buf_j,k) * reduced_mesh%weights)
          end do
        end do
        raw_i = raw_i + reduced_mesh%reduce_factor
      end do
      call fill_zonal_halo_member(block, reduced_mesh%halo_width, reduced_state%mf_lon_n(:,:,:,buf_j,move), east_halo=.false.)
    end if
  ...
  subroutine reduce_mf_lat_n(j, buf_j, move, block, raw_mesh, raw_state, reduced_mesh, reduced_static, reduced_state, dt)
    ...
    if (raw_mesh%is_inside_with_halo_half_lat(j+buf_j)) then
      ...
      do i = reduced_mesh%full_lon_ibeg, reduced_mesh%full_lon_iend
        do k = reduced_mesh%full_lev_ibeg, reduced_mesh%full_lev_iend
          do im = 1 , member_num
            reduced_state%mf_lat_n(im,k,i,buf_j,move) = sum(raw_state%mf_lat_n(im,raw_i:raw_i+reduced_mesh%reduce_factor-1,j+buf_j,k) * reduced_mesh%weights)
          end do
        end do
        raw_i = raw_i + reduced_mesh%reduce_factor
      end do
      call fill_zonal_halo_member(block, reduced_mesh%halo_width, reduced_state%mf_lat_n(:,:,:,buf_j,move), west_halo=.false.)
    end if
  ...
  subroutine reduce_dpv_lon_t(j, buf_j, move, block, raw_mesh, raw_state, reduced_mesh, reduced_static, reduced_state, dt)
    ...
    call fill_zonal_halo_member(block, reduced_mesh%halo_width, reduced_state%dpv_lon_t(:,:,:,buf_j,move))
  ...
  subroutine reduce_dpv_lat_t(j, buf_j, move, block, raw_mesh, raw_state, reduced_mesh, reduced_static, reduced_state, dt)
    ...
    call fill_zonal_halo_member(block, reduced_mesh%halo_width, reduced_state%dpv_lat_t(:,:,:,buf_j,move))
  ...
  subroutine reduce_pv_lon_midpoint(j, buf_j, move, block, raw_mesh, raw_state, reduced_mesh, reduced_static, reduced_state, dt)
    ...
    call fill_zonal_halo_member(block, reduced_mesh%halo_width, reduced_state%pv_lon(:,:,:,buf_j,move), east_halo=.false.)
  ...
  subroutine reduce_pv_lon_apvm(j, buf_j, move, block, raw_mesh, raw_state, reduced_mesh, reduced_static, reduced_state, dt)
    ...
    real(r8) u(member_num), v(member_num), le, de
    integer i, k
  ...
  subroutine reduce_pv_lat_midpoint(j, buf_j, move, block, raw_mesh, raw_state, reduced_mesh, reduced_static, reduced_state, dt)
    ...
    call fill_zonal_halo_member(block, reduced_mesh%halo_width, reduced_state%pv_lat(:,:,:,buf_j,move), west_halo=.false.)
  ...
  subroutine reduce_ke(j, buf_j, move, block, raw_mesh, raw_state, reduced_mesh, reduced_static, reduced_state, dt)
    ...
    do i = reduced_mesh%full_lon_ibeg, reduced_mesh%full_lon_iend
      do k = reduced_mesh%full_lev_ibeg, reduced_mesh%full_lev_iend
        do im = 1 , member_num
          reduced_state%ke(im,k,i,buf_j,move) = sum(raw_state%ke(im,raw_i:raw_i+reduced_mesh%reduce_factor-1,j+buf_j,k) * reduced_mesh%weights)
        end do
      end do
      raw_i = raw_i + reduced_mesh%reduce_factor
    end do
    call fill_zonal_halo_member(block, reduced_mesh%halo_width, reduced_state%ke(:,:,:,buf_j,move), west_halo=.false.)
  ...
  subroutine reduce_pt(j, buf_j, move, block, raw_mesh, raw_state, reduced_mesh, reduced_static, reduced_state, dt)
    ...
    do i = reduced_mesh%full_lon_ibeg, reduced_mesh%full_lon_iend
      do k = reduced_mesh%full_lev_ibeg, reduced_mesh%full_lev_iend
        do im = 1 , member_num
          reduced_state%pt(im,k,i,buf_j,move) = sum(raw_state%pt(im,raw_i:raw_i+reduced_mesh%reduce_factor-1,j+buf_j,k) * reduced_mesh%weights)
        end do
      end do
      raw_i = raw_i + reduced_mesh%reduce_factor
    end do
    call fill_zonal_halo_member(block, reduced_mesh%halo_width, reduced_state%pt(:,:,:,buf_j,move))
  ...
  subroutine reduce_ptf_lon(j, buf_j, move, block, raw_mesh, raw_state, reduced_mesh, reduced_static, reduced_state, dt)
    ...
    call fill_zonal_halo_member(block, reduced_mesh%halo_width, reduced_state%ptf_lon(:,:,:,buf_j,move), east_halo=.false.)
  ...
  subroutine reduce_t(j, buf_j, move, block, raw_mesh, raw_state, reduced_mesh, reduced_static, reduced_state, dt)
    ...
    call fill_zonal_halo_member(block, reduced_mesh%halo_width, reduced_state%t(:,:,:,buf_j,move))
  ...
  subroutine reduce_ak_t_lon(j, buf_j, move, block, raw_mesh, raw_state, reduced_mesh, reduced_static, reduced_state, dt)
    ...
    call fill_zonal_halo_member(block, reduced_mesh%halo_width, reduced_state%ak(:,:,:,buf_j,move))
  ...
  subroutine reduce_ph_lev(j, buf_j, move, block, raw_mesh, raw_state, reduced_mesh, reduced_static, reduced_state, dt)
    ...
    do i = reduced_mesh%full_lon_ibeg, reduced_mesh%full_lon_iend
      do k = reduced_mesh%half_lev_ibeg, reduced_mesh%half_lev_iend
        do im = 1 , member_num
          reduced_state%ph_lev(im,k,i,buf_j,move) = sum(                              &
            raw_state%ph_lev(im,raw_i:raw_i+reduced_mesh%reduce_factor-1,j+buf_j,k) * &
    ...
    call fill_zonal_halo_member(block, reduced_mesh%halo_width, reduced_state%ph_lev(:,:,:,buf_j,move), west_halo=.false.)
  ...
  subroutine reduce_append_array(move, reduced_mesh, reduced_array, raw_mesh, raw_array)
    ...
    integer, intent(in) :: move
    type(reduced_mesh_type), intent(in) :: reduced_mesh
    real(r8), intent(in) :: reduced_array(member_num , reduced_mesh%full_lon_lb:reduced_mesh%full_lon_ub)
    type(mesh_type), intent(in) :: raw_mesh
    real(r8), intent(inout) :: raw_array(member_num , raw_mesh%full_lon_lb:raw_mesh%full_lon_ub)
    ...
    do i = reduced_mesh%full_lon_ibeg, reduced_mesh%full_lon_iend
      do im = 1 , member_num
        raw_array(im,raw_i:raw_i+reduced_mesh%reduce_factor-1) = raw_array(im,raw_i:raw_i+reduced_mesh%reduce_factor-1) + reduced_array(im,i) * reduced_mesh%weights
      end do
      raw_i = raw_i + reduced_mesh%reduce_factor
    end do
  ...
  subroutine allocate_reduced_static(reduced_mesh, reduced_static , member_num)

    type(reduced_mesh_type), intent(in) :: reduced_mesh
    type(reduced_static_type), intent(inout) :: reduced_static
    integer , intent(in) :: member_num

    allocate(reduced_static%gzs(member_num , reduced_mesh%full_lon_lb:reduced_mesh%full_lon_ub, -2:2,reduced_mesh%reduce_factor))

  end subroutine allocate_reduced_static

  subroutine allocate_reduced_tend(reduced_mesh, reduced_tend, member_num)
    ...
    allocate(reduced_tend%qhu     (member_num,is:ie,ks:ke))
    allocate(reduced_tend%fu      (member_num,is:ie,ks:ke))
    allocate(reduced_tend%dmfdlon (member_num,is:ie,ks:ke))
    allocate(reduced_tend%dptfdlon(member_num,is:ie,ks:ke))

    is = reduced_mesh%half_lon_lb; ie = reduced_mesh%half_lon_ub
    ks = reduced_mesh%full_lev_lb; ke = reduced_mesh%full_lev_ub

    allocate(reduced_tend%qhv     (member_num,is:ie,ks:ke))
    allocate(reduced_tend%fv      (member_num,is:ie,ks:ke))
    allocate(reduced_tend%pgf_lon (member_num,is:ie,ks:ke))
    allocate(reduced_tend%dkedlon (member_num,is:ie,ks:ke))
  ...
  subroutine reduce_static(j, block, raw_mesh, raw_static, reduced_mesh, reduced_static)
    ...
    do move = 1, reduced_mesh%reduce_factor
      do buf_j = lbound(reduced_static%gzs, 3), ubound(reduced_static%gzs, 3)

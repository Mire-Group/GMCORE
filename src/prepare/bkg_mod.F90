      ...
      associate (mesh => proc%blocks(iblk)%mesh, &
        ...
        do j = mesh%full_lat_ibeg, mesh%full_lat_iend
          do i = mesh%full_lon_ibeg, mesh%full_lon_iend
            ...
            phs(1,i,j) = p0(i,j) * (1.0_r8 - lapse * (gzs(1,i,j) / g - z0(i,j)) / t0(i,j))**(1.0_r8 / lapse_kappa)
          end do
        end do
    ...
    do iblk = 1, size(proc%blocks)
      associate (mesh => proc%blocks(iblk)%mesh        , &
        ...
        do k = mesh%full_lev_ibeg, mesh%full_lev_iend
          do j = mesh%full_lat_ibeg, mesh%full_lat_iend
            do i = mesh%full_lon_ibeg, mesh%full_lon_iend
              ph(1,i,j,k) = vert_coord_calc_ph(k, phs(1,i,j))
            end do
          end do
        end do
      end associate
    ...
    do iblk = 1, size(proc%blocks)
      associate (block => proc%blocks(iblk)             , &
        ...
        case ('era5')
          ...
          do j = mesh%full_lat_ibeg, mesh%full_lat_iend
            do i = mesh%full_lon_ibeg, mesh%full_lon_iend
              call vert_interp_log_linear(era5_lev, tmp(i,j,:), ph(1,i,j,:), t(1,i,j,:), allow_extrap=.true.)
              pt(1,i,j,:) = potential_temperature(t(1,i,j,:), ph(1,i,j,:))
            end do
          end do
        case ('mpas')
          ...
          do j = mesh%full_lat_ibeg, mesh%full_lat_iend
            do i = mesh%full_lon_ibeg, mesh%full_lon_iend
              call vert_interp_log_linear(mpas_p(i,j,:), tmp(i,j,:), ph(1,i,j,:), pt(1,i,j,:), allow_extrap=.true.)
            end do
          end do
        end select
        deallocate(tmp)
        call fill_halo_member(block, pt, full_lon=.true., full_lat=.true., full_lev=.true.)
      end associate
    end do
        ...
        case ('era5')
          ...
          do j = mesh%full_lat_ibeg, mesh%full_lat_iend
            do i = mesh%half_lon_ibeg, mesh%half_lon_iend
              call vert_interp_linear(era5_lev, tmp(i,j,:), ph(1,i,j,:), u(1,i,j,:), allow_extrap=.true.)
            end do
          end do
        case ('mpas')
          ...
          do j = mesh%full_lat_ibeg, mesh%full_lat_iend
            do i = mesh%half_lon_ibeg, mesh%half_lon_iend
              call vert_interp_linear(mpas_p(i,j,:), tmp(i,j,:), ph(1,i,j,:), u(1,i,j,:), allow_extrap=.true.)
            end do
          end do
        end select
        deallocate(tmp)
      ...
      associate (block => proc%blocks(iblk)             , &
        ...
        case ('era5')
          ...
          do j = mesh%half_lat_ibeg, mesh%half_lat_iend
            do i = mesh%full_lon_ibeg, mesh%full_lon_iend
              call vert_interp_linear(era5_lev, tmp(i,j,:), ph(1,i,j,:), v(1,i,j,:), allow_extrap=.true.)
            end do
          end do
        case ('mpas')
          ...
          do j = mesh%half_lat_ibeg, mesh%half_lat_iend
            do i = mesh%full_lon_ibeg, mesh%full_lon_iend
              call vert_interp_linear(mpas_p(i,j,:), tmp(i,j,:), ph(1,i,j,:), v(1,i,j,:), allow_extrap=.true.)
            end do
          end do
        end select
        deallocate(tmp)
        call fill_halo_member(block, v, full_lon=.true., full_lat=.false., full_lev=.true.)
      end associate
    end do

...
contains
  ...
  subroutine era5_reader_run(bkg_file)
    ...
    if ( .not. allocated(era5_lon))  allocate(era5_lon (num_era5_lon))
    if ( .not. allocated(era5_lat))  allocate(era5_lat (num_era5_lat))
    if ( .not. allocated(era5_lev))  allocate(era5_lev (num_era5_lev))
    if ( .not. allocated(era5_u))    allocate(era5_u   (num_era5_lon,num_era5_lat,num_era5_lev))
    if ( .not. allocated(era5_v))    allocate(era5_v   (num_era5_lon,num_era5_lat,num_era5_lev))
    if ( .not. allocated(era5_t))    allocate(era5_t   (num_era5_lon,num_era5_lat,num_era5_lev))
    if ( .not. allocated(era5_ps))   allocate(era5_ps  (num_era5_lon,num_era5_lat             ))
    if ( .not. allocated(era5_mslp)) allocate(era5_mslp(num_era5_lon,num_era5_lat             ))

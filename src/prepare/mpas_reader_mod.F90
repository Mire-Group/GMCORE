...
contains
  ...
  subroutine mpas_reader_run(bkg_file)
    ...
    if ( .not. allocated(tmp))      allocate(tmp      (num_mpas_lon,num_mpas_lat,num_mpas_lev))
    if ( .not. allocated(mpas_lon)) allocate(mpas_lon (num_mpas_lon))
    if ( .not. allocated(mpas_lat)) allocate(mpas_lat (num_mpas_lat))
    if ( .not. allocated(mpas_u))   allocate(mpas_u   (num_mpas_lon,num_mpas_lat,num_mpas_lev))
    if ( .not. allocated(mpas_v))   allocate(mpas_v   (num_mpas_lon,num_mpas_lat,num_mpas_lev))
    if ( .not. allocated(mpas_pt))  allocate(mpas_pt  (num_mpas_lon,num_mpas_lat,num_mpas_lev))
    if ( .not. allocated(mpas_t))   allocate(mpas_t   (num_mpas_lon,num_mpas_lat,num_mpas_lev))
    if ( .not. allocated(mpas_p))   allocate(mpas_p   (num_mpas_lon,num_mpas_lat,num_mpas_lev))
    if ( .not. allocated(mpas_ps))  allocate(mpas_ps  (num_mpas_lon,num_mpas_lat))
    if ( .not. allocated(mpas_zs))  allocate(mpas_zs  (num_mpas_lon,num_mpas_lat))

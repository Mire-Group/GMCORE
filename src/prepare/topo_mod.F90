...
contains
      ...
      do j = mesh%full_lat_ibeg_no_pole, mesh%full_lat_iend_no_pole
        ...
        do i = mesh%full_lon_ibeg, mesh%full_lon_iend
          ...
          if (i1 > i2) then
            ...
            i2 = num_topo_lon
            call count_topo_grids(gzs0, sgh0, landfrac0, grid_count, i1, i2, j1, j2)
            gzs(1,i,j) = gzs(1,i,j) + gzs0
            grid_count0 = grid_count + grid_count0
            gzs(1,i,j) = gzs(1,i,j) / grid_count0
          else
            call count_topo_grids(gzs0, sgh0, landfrac0, grid_count, i1, i2, j1, j2)
            gzs(1,i,j) = gzs0 / grid_count
          end if
        end do
      end do
  ...
  subroutine topo_zero(block, min_lon, max_lon, min_lat, max_lat)
    ...
    associate (mesh => block%mesh, gzs => block%static%gzs)
      ...
      gzs(1,ibeg:iend,jbeg:jend) = 0.0_r8
    end associate
    ...
    associate (mesh => block%mesh, gzs => block%static%gzs)
      ...
      do step = 1, steps
        do j = jbeg, jend
          do i = ibeg, iend
            if (gzs(1,i,j) == 0) then
              smoothed_gzs(i,j) = gzs(1,i,j)
            else
              smoothed_gzs(i,j) = sum(smooth_kernel * gzs(1,i-5:i+5,j-5:j+5))
            end if
          end do
        end do
        gzs(1,ibeg:iend,jbeg:jend) = smoothed_gzs(ibeg:iend,jbeg:jend)
      end do
